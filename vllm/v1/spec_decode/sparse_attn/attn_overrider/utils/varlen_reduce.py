...
// Masked keys are stored as -inf -> softmax_scale * -inf - lse = -inf -> 0.
__device__ __forceinline__ float weight_from_score(
    float score, float softmax_scale, float lse) {
    return __expf(softmax_scale * score - lse);
}
...
__global__ void VarlenMeanReduceKernel(
    ...
    auto val = [&](float score, int64_t ho, int32_t tok) -> float {
        if constexpr (USE_WEIGHT) {
            return weight_from_score(score, softmax_scale, lse[ho + tok]);
        } else {
            return score;
        }
    };
...
__global__ void VarlenMeanReduceKernelVec(
    ...
    // Accumulate one slot (uint4-loaded 8 scores) into s[]. lse_val is only
    // dereferenced in weight mode.
    auto accum_slot = [&](int64_t off, float lse_val) {
        const uint4 v = __ldg(
        ...
        for (int i = 0; i < kVecWidth; ++i) {
            ...
            if constexpr (USE_WEIGHT) {
                s[i] += weight_from_score(sc, softmax_scale, lse_val);
            } else {
                s[i] += sc;
            }
        }
    ...
    if (entry == 0) {
        for (int32_t h = 0; h < dim1; ++h) {
            ...
            const float lse0 = USE_WEIGHT ? lse[ho + first_tok] : 0.f;
            const float lse1 = USE_WEIGHT ? lse[ho + last_tok] : 0.f;
